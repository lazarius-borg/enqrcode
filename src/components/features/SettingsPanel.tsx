        ...
        <div className="w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-xs uppercase font-bold text-text-dim tracking-wider">Resolution (Output Size)</h3>
                </div>

                <div className="bg-surface border border-md-outline-variant rounded-2xl p-4">
                    <div className="flex justify-between items-center mb-4">
                        <span className="text-sm font-medium text-md-outline">Export Quality</span>
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-xs uppercase font-bold text-text-dim tracking-wider">Error Correction</h3>
                </div>
                    ...
                    {eccLevels.map((level) => (
                        ...
                        >
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${(options.errorCorrectionLevel || 'M') === level.id
                                ...
                                : 'bg-md-surface-variant text-text-dim'
                                }`}>
                                {level.id}
                            </div>
                            <div>
                                <div className={`text-sm font-medium ${(options.errorCorrectionLevel || 'M') === level.id ? 'text-white' : 'text-md-outline'
                                    }`}>
                                    {level.label}
                                </div>
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-xs uppercase font-bold text-text-dim tracking-wider">File Format</h3>
                </div>
                    ...
                    {['png', 'jpeg', 'svg'].map((fmt) => (
                        <button
                            ...
                            className={`py-3 rounded-xl border font-medium text-sm transition-all uppercase ${(options.fileFormat || 'png') === fmt
                                ...
                                : 'bg-surface border-md-outline-variant text-text-dim hover:text-text-main hover:bg-surface-hover'
                                }`}
                        >
                            {fmt}
                        </button>
